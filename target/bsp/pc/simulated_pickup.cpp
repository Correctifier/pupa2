...
void SimulatedPickup::set_control(float frequency_hz, float amplitude_v) {
  generator_frequency_hz_=frequency_hz; generator_amplitude_v_=amplitude_v;
...
float SimulatedPickup::sense_resistor_ohm() const { return static_cast<float>(ranges[range_index_]); }

bool SimulatedPickup::set_range_manual(std::uint32_t range_index) {
  if(range_index>=std::size(ranges)) return false;
  automatic_range_=false; range_index_=range_index; return true;
}

bool SimulatedPickup::start_acquisition(std::uint16_t* buffer, std::size_t count) {
  if(acquisition_active_) return false;
  if(buffer==nullptr || count<2 || count%2!=0) return false;
  ...
  const auto impedance=1.0/(1.0/series+std::complex<double>(0,omega*parameters_.capacitance_pf*1e-12));
  if(automatic_range_) {
    double best=std::numeric_limits<double>::max();
    for(std::uint32_t i=0;i<std::size(ranges);++i) {
      const double error=std::abs(std::log(ranges[i]/std::abs(impedance)));
      if(error<best) { best=error; range_index_=i; }
    }
  }
  ...
  dma_buffer_=buffer; dma_buffer_count_=count; clean_count_=0; acquisition_active_=true;
  return true;
}

void SimulatedPickup::advance_dma() const {
  if(!acquisition_active_ || clean_count_==dma_buffer_count_) return;
  ...
  for(std::size_t i=clean_count_;i<end;i+=2) {
    const std::size_t sample=i/2;
    ...
    const double sense=std::real(vsense_signal_*carrier)+std::abs(vsense_signal_)*noise(generator_);
    const auto quantize=[](double value) { return static_cast<std::uint16_t>(std::clamp(std::lround(2048.0+value*adc_counts_per_volt),0L,4095L)); };
    dma_buffer_[i]=quantize(v); dma_buffer_[i+1]=quantize(sense);
  }
  clean_count_=end;
  if(clean_count_==dma_buffer_count_) acquisition_active_=false;
}

std::size_t SimulatedPickup::clean_data_count() const { advance_dma(); return clean_count_; }
