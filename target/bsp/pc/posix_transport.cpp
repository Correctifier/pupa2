...
PosixTransport::PosixTransport(std::uint16_t tcp_port) {
  ...
  if (server_fd_ < 0) throw std::runtime_error("could not create TCP socket");
  int reuse = 1;
  ...
  if (pty_master_fd_ >= 0 && grantpt(pty_master_fd_) == 0 &&
      unlockpt(pty_master_fd_) == 0) {
    if (const char* path = ptsname(pty_master_fd_)) serial_path_ = path;
    endpoints_[1] = pty_master_fd_;
  }
}

PosixTransport::~PosixTransport() {
  for (const auto& [_, fd] : endpoints_) close(fd);
  ...
  for (const auto& [endpoint, fd] : endpoints_) {
    while (true) {
      ...
      if (count > 0) {
        ...
        if (count == 0 && endpoint != 1) closed.push_back(endpoint);
        break;
      }
    }
  }
  for (auto endpoint : closed) close_endpoint(endpoint);
}
...
void PosixTransport::send(std::uint32_t endpoint, std::string_view line) {
  ...
  while (remaining > 0) {
    const auto sent = write(found->second, data, remaining);
    if (sent > 0) {
      data += sent;
