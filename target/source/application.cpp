...
void Application::tick() {
  while (auto line = dependencies_.transport.receive()) {
    ...
    const auto request = protocol::parse_request(
        line->view(),
        code,
        message,
        &envelope
    ...
    if (request->object == protocol::Object::device && request->action == protocol::Action::info) {
      send_message(
          ...
          protocol::device_info_response(
              *request,
              dependencies_.device.target_name,
...
void Application::process_sweep() {
  ...
  const auto result = sweep.processor.finish(
  ...
  if (!result) {
    send_message(
        ...
        protocol::error_response(
            0,
            protocol::Object::measurement,
            protocol::Action::acquire,
            "invalid_signal",
