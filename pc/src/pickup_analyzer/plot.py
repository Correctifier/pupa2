...
class SweepPlot(tk.Canvas):
    COLORS = ("#4da3ff", "#ff9f43", "#58d68d", "#d980fa", "#ff6b6b", "#48dbfb")

    def __init__(self, master, title: str, x_label: str, y_label: str,
                 series_builder: Callable[[Sweep, str], list[Series]], log_x: bool = False,
                 log_y: bool = False, y_range: tuple[float, float] | None = None,
                 equal_units: bool = False):
        super().__init__(master, background="#15181d", highlightthickness=0, height=430)
    ...
    def redraw(self) -> None:
        ...
        self.create_text(width / 2, 17, text=self.title, fill="#f1f3f5",
                         font=("TkDefaultFont", 11, "bold"))
        self.create_text(width / 2, height - 17, text=self.x_label, fill="#b9c0c9")
        self.create_text(15, (top + bottom) / 2, text=self.y_label, fill="#b9c0c9", angle=90)
        series: list[Series] = []
        color_index = 0
        ...
        values = [(x, y) for _, _, points in series for x, y in points
                  if (x > 0 or not self.log_x) and (y > 0 or not self.log_y)]
        if not values:
            self.create_text(width / 2, height / 2, text="No sweep data", fill="#747d8c")
            return
        transformed = [(math.log10(x) if self.log_x else x,
                        math.log10(y) if self.log_y else y) for x, y in values]
        x_min, x_max = min(x for x, _ in transformed), max(x for x, _ in transformed)
        if self.y_range is None:
            y_min, y_max = self._padded(min(y for _, y in transformed), max(y for _, y in transformed))
        else:
        ...
        if self.equal_units:
            ...
            units_per_pixel = max((x_max - x_min) / plot_width,
        ...
        def screen(x: float, y: float) -> tuple[float, float]:
            ...
            return (left + (tx - x_min) / (x_max - x_min) * (right - left),
                    bottom - (ty - y_min) / (y_max - y_min) * (bottom - top))

        for index in range(6):
            fraction = index / 5
            x, y = left + fraction * (right - left), bottom - fraction * (bottom - top)
            self.create_line(x, top, x, bottom, fill="#2b3038")
            self.create_line(left, y, right, y, fill="#2b3038")
            xv = 10 ** (x_min + fraction * (x_max - x_min)) if self.log_x else x_min + fraction * (x_max - x_min)
            yv_transformed = y_min + fraction * (y_max - y_min)
            yv = 10 ** yv_transformed if self.log_y else yv_transformed
            self.create_text(x, bottom + 16, text=f"{xv:.3g}", fill="#929aa5", font=("TkDefaultFont", 8))
            self.create_text(left - 8, y, text=f"{yv:.3g}", fill="#929aa5", anchor="e", font=("TkDefaultFont", 8))
        self.create_rectangle(left, top, right, bottom, outline="#59616d")
        legend_y = top + 8
...
def magnitude_series(sweep: Sweep, color: str) -> list[Series]:
    return [(sweep.name, color, [(p.frequency_hz, p.magnitude_ohm) for p in sweep.points])]


def phase_series(sweep: Sweep, color: str) -> list[Series]:
    return [(sweep.name, color, [(p.frequency_hz, p.phase_degrees) for p in sweep.points])]
...
def components_series(sweep: Sweep, color: str) -> list[Series]:
    ...
    return [(f"{sweep.name} Re", color, [(p.frequency_hz, p.real_ohm) for p in sweep.points]),
            (f"{sweep.name} Im", alternate, [(p.frequency_hz, p.imaginary_ohm) for p in sweep.points])]


def nyquist_series(sweep: Sweep, color: str) -> list[Series]:
    return [(sweep.name, color, [(p.real_ohm, p.imaginary_ohm) for p in sweep.points])]
