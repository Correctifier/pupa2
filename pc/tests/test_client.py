...
class FakeTransport(Transport):
    ...
    def send(self, message):
        ...
        self.incoming.put({"type": "response", "object": message["object"],
                           "action": message["action"], "id": message["id"], "status": "ok"})
    def receive(self):
...
class ClientTests(unittest.TestCase):
    ...
    def test_routes_measurement_event(self):
        transport = FakeTransport()
        client = AnalyzerClient(transport)
        transport.incoming.put({"type":"event", "object":"measurement", "data":{
            "f":1000, "range":2, "rsense":10000, "v":{"re":.2,"im":0},
            "vsense":{"re":.04,"im":-.01}, "v_min":1,"v_max":2,
            "vsense_min":3,"vsense_max":4,"z":{"re":7000,"im":18000}}})
